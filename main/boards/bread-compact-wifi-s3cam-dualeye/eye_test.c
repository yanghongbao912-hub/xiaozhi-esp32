...
static void test_task(void *arg) {
    ...
    while (1) {
        ...
        // GC9A01 driver (SELLER's firmware uses this!)
        ...
        esp_lcd_panel_invert_color(panel, true);  // IPS 反色(卖家示例也开了)
        ESP_LOGI(TAG, "GC9A01 driver: MOSI=%d SCK=%d CS=%d DC=%d RST=%d", mosi, sck, cs, dc, rst);

        // Draw 160x160 at (0,0) RED, then (40,40) GREEN - covers window offset
        draw_solid(panel, 0xF800, 0, 0, 160, 160);
        vTaskDelay(pdMS_TO_TICKS(5000));
        draw_solid(panel, 0x07E0, 40, 40, 160, 160);
        vTaskDelay(pdMS_TO_TICKS(5000));
        draw_solid(panel, 0x001F, 0, 0, 160, 160);
        vTaskDelay(pdMS_TO_TICKS(5000));
...
void eye_test_permutation_start(void) {
    ESP_LOGI(TAG, "starting GC9A01-driver test (SELLER's driver) 160x160 draws");
